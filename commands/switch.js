...
const { execGitWithSpinner, handleCancel } = require('../lib/utils/command-helpers');
  ...
  if (!name) {
    ...
    const branchOptions = branches.local.map((branch) => ({
      ...
      label: branch.current
        ? `${chalk.green('*')} ${chalk.green.bold(branch.name)}`
        : branch.name,
    }));

    name = await clack.select({
      message: chalk.cyan('Select branch to switch to:'),
