...
module.exports = async (args) => {
  ...
  requireTTY('Please use: git archive --format=<format> --output=<file> <tree-ish> for non-interactive mode');
  ...
  if (!selectedFormat) {
    selectedFormat = await clack.select({
      message: chalk.cyan('Archive format:'),
      options: [
  ...
  if (!outputFile) {
    ...
    outputFile = await clack.text({
      message: chalk.cyan('Output file:'),
