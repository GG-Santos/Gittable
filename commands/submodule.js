...
const addSubmodule = async (args) => {
  ...
  if (!url) {
    url = await clack.text({
      message: chalk.cyan('Submodule repository URL:'),
  ...
  if (!path) {
    ...
    path = await clack.text({
      message: chalk.cyan('Submodule path:'),
...
const removeSubmodule = async (args) => {
  ...
  if (!path) {
    ...
    if (listResult.success && listResult.output.trim()) {
      ...
      const submoduleOptions = lines.map((line) => {
        const match = line.match(/[\s-+U][a-f0-9]+\s+(.+?)(\s+\(.+\))?$/);
        return match ? {
          value: match[1],
          label: match[1],
      ...
      path = await clack.select({
        message: chalk.cyan('Select submodule to remove:'),
