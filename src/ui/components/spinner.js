...
const cliCursorModule = require('cli-cursor');
const cliCursor = cliCursorModule.default || cliCursorModule;
const cliSpinnersModule = require('cli-spinners');
const cliSpinners = cliSpinnersModule.default || cliSpinnersModule;
const logSymbolsModule = require('log-symbols');
const logSymbols = logSymbolsModule.default || logSymbolsModule;
...
const isInteractiveModule = require('is-interactive');
const isInteractive = isInteractiveModule.default || isInteractiveModule;

const isUnicodeSupportedModule = require('is-unicode-supported');
const isUnicodeSupported = isUnicodeSupportedModule.default || isUnicodeSupportedModule;

const stdinDiscarderModule = require('stdin-discarder');
const stdinDiscarder = stdinDiscarderModule.default || stdinDiscarderModule;
...
async function spinnerPromise(action, options) {
	...
	const spinnerInstance = spinner(options).start();

	try {
		const promise = actionIsFunction ? action(spinnerInstance) : action;
		const result = await promise;

		spinnerInstance.succeed(successText === undefined
			? undefined
			: (typeof successText === 'string' ? successText : successText(result)));

		return result;
	} catch (error) {
		spinnerInstance.fail(failText === undefined
			? undefined
...
module.exports.spinnerPromise = spinnerPromise;
module.exports.spinners = cliSpinners;
